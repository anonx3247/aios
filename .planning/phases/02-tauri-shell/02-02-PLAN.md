...
<task type="auto">
  <name>Task 2: SQLite database initialization with migrations</name>
  <files>
    apps/desktop/src-tauri/src/lib.rs
  </files>
  <action>
    1. Update src/lib.rs to add tauri-plugin-sql with SQLite migrations:
       ```
    ...
    2. Add `mod services;` to lib.rs module declarations.

    IMPORTANT: The key list file for KeyringService should be stored in the app's data directory. Use app.path().app_data_dir() to get the path. Pass this path when constructing KeyringService in setup.
  </action>
  <verify>
    `cd /Users/neosapien/dev/aios/apps/desktop/src-tauri && cargo check` compiles.
  </verify>
  <done>SQLite plugin registered with migrations for runs and messages tables. Services module declared.</done>
</task>

<task type="auto">
  <name>Task 3: Wire secrets commands and state initialization into Tauri builder</name>
  <files>
    apps/desktop/src-tauri/src/lib.rs
    apps/desktop/src-tauri/capabilities/default.json
  </files>
  <action>
    1. Initialize AppState with KeyringService in setup closure, manage via app.manage().

    2. Register secrets commands in generate_handler! macro (alongside existing health_check):
    ...
    3. Update capabilities/default.json: Add allow permissions for all secret commands.

    IMPORTANT: Only ONE generate_handler! macro call with ALL commands listed.
  </action>
  <verify>
    `cd /Users/neosapien/dev/aios/apps/desktop/src-tauri && cargo build` succeeds. Run `pnpm tauri dev` and check that aios.db file is created in the app data directory.
  </verify>
